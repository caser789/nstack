...
func NonBlockingWrite(fd int, buf []byte) error {
	...
	n, _, e := syscall.Syscall(syscall.SYS_WRITE, uintptr(fd), uintptr(ptr), uintptr(len(buf)))
	if e != 0 {
		return e
	}
...
func NonBlockingWrite2(fd int, b1, b2 []byte) error {
	...
	n, _, e := syscall.RawSyscall(syscall.SYS_WRITE, uintptr(fd), uintptr(unsafe.Pointer(&iovec[0])), 2)
	if e != 0 {
		return e
	}
